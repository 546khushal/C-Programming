...
void Add(Student *x, int size, int finalsize) {
    int i;
    // Add the new records to the array
    for (i = size; i < finalsize; i++) {
	printf("Enter RollNo, Name, Marks for record %d: ", i + 1);
...
void main(){
	int ch,rno,sch,sortch,size=0,finalsize=0;
	Bool flag=false;
	Student *s;
	char nm[20];
	do{
	...
	switch(ch){
		...
		if (flag == false) {
		    ...
		    printf("\nEnter Record count to insert: ");
		    scanf("%d", &finalsize);
		    s = (Student *)realloc(s, (finalsize + size) * sizeof(Student)); // Resize the memory
		    printf("New final size: %d\n", finalsize + size);
		    Add(s, size, finalsize + size);
		    size = finalsize + size;
		}
		break;
		case 2://Display
